...
class FollowerFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (favorites != null){
            ...
            followerViewModel.setListUserFollower(activity!!.applicationContext,dataFavorite.username.toString())
            showLoading(true)

        }else{
         dataUser = activity!!.intent.getParcelableExtra(EXTRA_USER) as User

        followerViewModel.setListUserFollower(activity!!.applicationContext,dataUser.username.toString())
        showLoading(true)
        }
        showRecyclerView()
